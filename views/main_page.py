...
def _save(owner: Optional[Owner]) -> None:
...
saved_owners = storage.list_owner_names()
if saved_owners:
    st.caption("Previously saved owners: " + ", ".join(saved_owners))

owner_name = st.text_input("Owner name", value="Jordan")

if st.button("Set Owner"):
    existing = storage.load_owner(owner_name)
    if existing is not None:
        st.success(f"Loaded saved owner '{owner_name}' with {len(existing.get_pets())} pet(s).")
    else:
        _save(Owner(name=owner_name))
...
# --- Remove a pet ---
if owner is not None:
    removable_pets = [p.name for p in owner.get_pets()]
    if removable_pets:
...
if owner is None:
    ...
    with col6:
        if recurrence == "none":
            ...
            st.date_input("Due date", value=None, disabled=True, help="Recurring tasks always start from today")
            due = date.today()
    ...
    if st.button("Generate Schedule"):
        ...
        if not schedule:
            ...
            for task in schedule:
                ...
                due_display = "N/A" if task.recurrence != "none" else str(task.due_date)
                rows.append({
